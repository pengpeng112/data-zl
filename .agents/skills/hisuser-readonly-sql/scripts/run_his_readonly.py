#!/usr/bin/env python3
"""Run a bounded HIS_SOURCE SELECT through the repository OracleConnector.

Run from backend/. Credentials are read only from the process environment or a
controlled credential file. This script never accepts passwords as arguments.
"""
...
def load_credential() -> tuple[str, str]:
    user = os.environ.get("APP_HIS_SOURCE_USER", "").strip()
    password = os.environ.get("APP_HIS_SOURCE_PASSWORD", "")
    combined = os.environ.get("CRED_HIS_SOURCE", "")
    if combined and ":" in combined:
        user, password = combined.split(":", 1)
    if not (user and password):
        credential_file = Path(os.environ.get(
            "APP_HIS_CREDENTIAL_FILE",
    ...
    if not (user and password):
        raise RuntimeError("HIS read-only credential is not configured through the approved environment")
...
def redact_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in row.items():
        upper = str(key).upper()
        result[key] = "***" if any(marker in upper for marker in SENSITIVE_MARKERS) else normalize(value)
    return result


def sanitize_error(value: str) -> str:
    value = str(value)[:500]
    value = __import__("re").sub(r"\b(?:\d{1,3}\.){3}\d{1,3}\b", "<host>", value)
    value = __import__("re").sub(r"(?i)(user|password|pwd)\s*=\s*[^\s,;]+", r"\1=<redacted>", value)
    return value[:200]


def build_connector() -> OracleConnector:
    user, password = load_credential()
    mode = os.environ.get("APP_HIS_SOURCE_CONNECTION_MODE", "direct").strip().lower()
    if mode not in {"direct", "ssh_jump"}:
        raise RuntimeError("APP_HIS_SOURCE_CONNECTION_MODE must be direct or ssh_jump")
    return OracleConnector(
        host=os.environ.get("APP_HIS_SOURCE_HOST", "10.10.10.15"),
        port=int(os.environ.get("APP_HIS_SOURCE_PORT", "1521")),
        database=os.environ.get("APP_HIS_SOURCE_SERVICE", "his"),
        user=user,
        password=password,
        connection_mode=mode,
        oracle_client_lib_dir=os.environ.get("APP_ORACLE_CLIENT_LIB_DIR", "/opt/oracle"),
        timeout_ms=min(max(int(os.environ.get("APP_HIS_QUERY_TIMEOUT_MS", "60000")), 1000), 120000),
        large_tables=list({
            "ORDADM.ORDERS", "LAB.LAB_RESULT", "INPBILL.INP_BILL_DETAIL",
            "OUTPBILL.OUTP_BILL_ITEMS", "ORDADM.ORDERS_EXECUTE_DETAILS",
        }),
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Execute a bounded, read-only HIS_SOURCE query")
