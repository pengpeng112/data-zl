...
class OpsToolTemplate(Base):
    ...
    sql_hash = Column(Text)
    created_by = Column(Text)
    updated_by = Column(Text)
    reviewed_by = Column(Text)
    reviewed_at = Column(TIMESTAMP(timezone=True))
    max_affected_rows = Column(Integer, server_default="100")
    target_scope = Column(Text, server_default="platform_asset")
    immutable_after_approval = Column(Boolean, server_default="true")
    target_connection_id = Column(BigInteger)
    target_source_code = Column(Text)
    target_database_key = Column(Text)
    target_schema = Column(Text)
...
class OpsToolRun(Base):
    ...
    sql_hash = Column(Text)
    preview_count = Column(Integer)
    confirmation_digest = Column(Text)
    transaction_id = Column(Text)
    error_code = Column(Text)
    error_summary_masked = Column(Text)
    target_connection_id = Column(BigInteger)
    target_source_code = Column(Text)
    target_database_key = Column(Text)
    target_schema = Column(Text)
    correlation_id = Column(Text)
    confirmation_expires_at = Column(TIMESTAMP(timezone=True))
    preview_params_hash = Column(Text)
...
class OpsEventLog(Base):
    """Normalized ops/connection/dict event stream (masked summaries only)."""

    __tablename__ = "asset_ops_event_logs"
    __table_args__ = {"schema": "asset"}
