...
- 日常取数先由 `query-governance-intake` 检索现有查询资产；只有目标物理来源明确后，才加载对应 SQL 技能补充方言和业务限制。
- 连接、系统定位和表/字段结构由 `sjzc` 负责；用户上下文已能唯一定位系统时直接查询该系统，不先列全部系统。平台当前元数据优先，离线资产仅在平台不可用或需要历史证据时回退。
- 历史 SQL 的 JOIN 摄取由 `sql-relation-intake` 负责；它可调用来源 SQL 技能，但不因此自动获得活库执行、候选发布或正式关系批准权限。
- 同一入口文件和未变化的技能在同一任务中无需重复读取；新增范围或证据发生变化时再补读。
...
**Track A 已完成**：AI 探库（数据中心 8.216 + HIS）→ 表结构与表间关系 → 关系图谱。
- 当前任务范围和开放项以 `55_系统未完成事项统一执行计划.md` 的最新状态及用户本次要求为准。历史计划、执行提示词和既往授权只作证据，不自动恢复已完成任务，也不构成新的生产发布、写库或 Git 授权。
...
1. **本机 Windows 不直连业务源库**，使用平台登记的 8.83 受控连接器或既有批准跳板（见 `02` §2）。平台元数据、隔离测试库和业务源库须按连接身份分别判断，不把一种通道的授权外推到另一种。账号密码不要写入新代码仓库；内部凭据按既有安全渠道获取。
...
- 对业务源库的批量探查在夜间执行并加超时；离线分析、平台已采集元数据查询和不连接业务源库的静态检查不受夜间窗口限制。白天 live 查询如无既有授权，只准备限量 SQL 和执行计划。

## 自主执行、澄清与批准边界

- 用户已明确要求修改、修复、开发或验证时，完成范围内可逆的本地编辑、静态检查和适当测试，不在阶段之间重复请求确认。只有缺失信息会改变业务目标、数据粒度、授权范围或不可逆结果，且无法从当前证据合理推断时才提问。
- 遇到局部依赖缺失时，只停止依赖该条件的步骤；继续其他已授权且可独立完成的工作，并在交付中列明受阻步骤、证据和恢复条件。不得通过改用生产库、降低校验、猜测字段或扩大范围来绕过阻塞。
- 明确批准继续保留：业务源库写入；生产平台 apply/迁移/发布；生产权限、cron 和运行配置调整；关系/值域/查询/指标/产品的人工治理终态；Git push/tag；以及具体计划显式列出的不可逆动作。会话中已授予且范围等价的批准无需重复索取，历史会话中的一次性授权不得自动沿用。
- 本地代码和文档编辑、只读检查、纯逻辑测试，以及对已核实隔离测试库的任务内验收，不等同于生产授权。测试数据库必须明确标识为允许清理的隔离库。

## 文档管理与目录维护（强制，每次会话执行）

> 文档量大且持续增长，必须维护**单一权威目录**（`开发起步包/README.md`）。该文件同时记录状态、用途、归档和更新历史；禁止再新建平行状态总表。
> **任何 AI（含本助手）每次会话开始时先做只读目录自检；每次新增/废弃文档必须同步清单。** 审阅任务只报告差异；编辑任务只修正本次范围内或已核实的目录问题，不因无关目录差异阻塞正文。

### 1. 启动自检（会话第一步，必做）
- 读 `开发起步包/README.md`（权威目录）与 `55_系统未完成事项统一执行计划.md`。
- 用 Glob 列 `开发起步包/*` 实际文件，与 README 各分组比对；识别同号正文、`_结果.json`、输出目录等配套资产，避免只按文件名全文匹配误报：
  - **孤儿文件**（实际存在、清单未登记）：先报告并核实；属于本次新增的立即补登记，确认被替代且无引用的才迁入 `_archive/`。
  - **幽灵条目**（清单有、文件不存在）：修正或删除该行。
- 与本次任务相关的不一致先修正；无关差异记录后继续正文工作，不擅自修改或归档他人资产。
...
1. **业务源库只读**：所有业务源库连接默认只读 `SELECT`，禁止 DML/DDL。业务写操作必须通过 `asset_action_executors` 白名单 + 审批。平台库、隔离测试库和 Alembic 验收按下述边界处理，不能与业务源库混为一谈。
...
5. **AI 工具写边界**：产品内 AI 工具仅调用只读探查端点并强制审计，不接入 `asset_action_executors` 写操作执行器。开发代理可在用户授权的开发任务内修改代码，并在已核实的隔离测试库验收；这不授权生产 apply、发布、权限/cron/env 调整或业务源库写入。自动审计不等于 AI 直接修改审计记录，禁止关闭或绕过审计。
...
按变更影响选择验收，不要求无关模块重复跑全量门禁：文档/提示词变更做引用、结构与行为用例检查；纯逻辑变更先跑无数据库测试；数据库集成测试和迁移只能在已核实、允许清理的隔离测试库执行。根级 pytest 的 autouse fixture 会执行 `TRUNCATE ... RESTART IDENTITY CASCADE`，运行前必须显式核实 `APP_TEST_DB_URL`，不得指向生产、开发或共享不可清理数据库。隔离库不可用时继续完成独立工作，只把依赖数据库的验收标为受阻。
