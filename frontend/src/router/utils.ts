...
/** 同时按角色与权限码过滤菜单；配置了 auths 的菜单必须命中至少一个权限。 */
function filterNoPermissionTree(data: RouteComponent[]) {
  ...
  const newTree = cloneDeep(data).filter((v: any) => {
    ...
    const required = v.meta?.auths as string[] | undefined;
    const permissionAllowed =
      !required?.length ||
      currentPermissions.includes("*:*:*") ||
      required.some(code => currentPermissions.includes(code));
...
function getHistoryMode(routerHistory = "hash"): RouterHistory {
  ...
  // 生产环境可能未注入 VITE_ROUTER_HISTORY；默认 hash，避免路由初始化阶段崩溃。
...
function getAuths(): Array<string> {
  return (router.currentRoute.value.meta.auths as Array<string> | undefined) ?? [];
}

/** 是否有按钮级别的权限（根据当前登录用户的实际 permissions 判断）*/
function hasAuth(value: string | Array<string>): boolean {
  if (!value) return false;
  const profile = storageLocal().getItem<DataInfo<number>>(userKey);
  return hasUserPermissions(value, profile);
}
