...
> 2026-07-31 复核：本机已恢复使用 `C:\Users\Administrator\.ssh\id_ed25519_ai` 对 `root@10.10.8.83:22` 的公钥免密访问，并以 `BatchMode=yes` 验证成功。后续 AI 应优先使用该密钥；不得在命令、文档或日志中记录 SSH 密码。若公钥失效，再由用户在可见终端交互输入密码恢复 `authorized_keys`。
