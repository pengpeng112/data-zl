...
## 2026-08-31 生产可用性复核补充

生产复核发现现行正式关系状态包含 `verified`/`sample_verified`，已纳入 AI 上下文；上下文构建改按完整 prompt 的 24000-byte 限制收缩，前端生成请求超时调整为 120 秒并展示后端错误详情。线上真实生成返回 200，`executed=false`，未执行生成 SQL。
